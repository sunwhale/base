...
sudo mount -t iso9660 DS.SIMULIA.2024.Ubuntu.iso /media # 挂载Abaqus光盘

sudo bash /media/4/SIMULIA_FLEXnet_LicenseServer/Linux64/1/StartTUI.sh # 执行过程中勾选 > Files only - do not start the license server program.
sudo bash /media/5/SIMULIA_EstablishedProducts/Linux64/1/StartTUI.sh # 安装过程中跳过许可证服务器的信息
sudo bash /media/6/SIMULIA_EstablishedProducts_CAA_API/Linux64/1/StartTUI.sh # 接口安装

sudo cp /media/ABAQUSLM__lmgrd__SSQ.lic /usr/SIMULIA/License/2024/linux_a64/code/bin/ABAQUSLM__lmgrd__SSQ.lic # 将许可证文件复制到许可证服务器安装路径
sudo mkdir /usr/tmp
sudo /usr/SIMULIA/License/2024/linux_a64/code/bin/lmgrd -c /usr/SIMULIA/License/2024/linux_a64/code/bin/ABAQUSLM__lmgrd__SSQ.lic # 启动许可证服务器
sudo /usr/SIMULIA/License/2024/linux_a64/code/bin/lmstat -c /usr/SIMULIA/License/2024/linux_a64/code/bin/ABAQUSLM__lmgrd__SSQ.lic -a # 查看许可证服务器状态
sudo sed -i '$alicense_server_type=FLEXNET' /usr/SIMULIA/EstProducts/2024/linux_a64/SMA/site/licensing.env # 修改配置文件
sudo sed -i '$aabaquslm_license_file=\"27800@localhost\"' /usr/SIMULIA/EstProducts/2024/linux_a64/SMA/site/licensing.env # 修改配置文件
...
## 3. 单机并行计算设置
修改/usr/SIMULIA/EstProducts/2024/linux_a64/SMA/site/basic_v6.env中的"mp_mode = MPI" -> "mp_mode = THREADS"
