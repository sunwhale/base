...
from base.forms.flow import UploadForm, FlowForm, JobForm, F1Form, F2Form
...
@flow_bp.route('/2', methods=['GET', 'POST'])
@permission_required('FLOW')
def f2():
    flow_id = 2
    flows_path = current_app.config['FLOW_PATH']
    ...
    if os.path.exists(flow_path):
        ...
        return render_template('flow/f2.html', flow_id=flow_id, status=status, form=form, upload_form=upload_form, job_form=job_form)
    else:
        abort(404)
