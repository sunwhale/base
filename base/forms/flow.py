...
class F2Form(FlaskForm):
    ...
    shell_theta_out_deg_front = FloatField('壳体头部外锥角 (度)', default=0.49)
    shell_theta_out_deg_behind = FloatField('壳体尾部外锥角 (度)', default=0.49)
    shell_r_out_at_a_front = FloatField('壳体头部轴心处外半径 (mm)', default=1797)
    shell_r_out_at_a_behind = FloatField('壳体尾部轴心处外半径 (mm)', default=1797)
    shell_theta_in_deg_front = FloatField('壳体头部内锥角 (度)', default=0.24)
    shell_theta_in_deg_behind = FloatField('壳体尾部内锥角 (度)', default=0.24)
    shell_r_in_front = FloatField('壳体前缘内半径 (mm)', default=562.5)
    shell_r_in_behind = FloatField('壳体尾缘内半径 (mm)', default=942.5)
    shell_l_c1_out = FloatField('壳体前缘距离前椭圆中心轴向距离 (mm)', default=1105.75)
    shell_l_c2_out = FloatField('壳体尾缘距离后椭圆中心轴向距离 (mm)', default=968.46)

    # 新增：绝缘层相关字段
    shell_insulation_theta_in_deg_front = FloatField('头部绝缘层内锥角 (度)', default=0.16)
    shell_insulation_theta_in_deg_behind = FloatField('尾部绝缘层内锥角 (度)', default=0.16)
    shell_insulation_r_in = FloatField('绝缘层中部内半径 (mm)', default=1764.5)
    shell_insulation_r_out = FloatField('绝缘层中部外半径 (mm)', default=1777.5)
    shell_insulation_r_in_at_a_front = FloatField('头部绝缘层轴心处内半径 (mm)', default=1762.5)
    shell_insulation_r_in_at_a_behind = FloatField('尾部绝缘层轴心处内半径 (mm)', default=1762.5)
    shell_insulation_r_in_front = FloatField('绝缘层前缘内半径 (mm)', default=425.0)
    shell_insulation_r_in_behind = FloatField('绝缘层尾缘内半径 (mm)', default=775.0)
    shell_insulation_thickness_at_flange_front = FloatField('绝缘层前缘法兰处厚度 (mm)', default=2.5)
    shell_insulation_thickness_at_flange_behind = FloatField('绝缘层尾缘法兰处厚度 (mm)', default=2.5)

    # 新增：盖板相关字段
    cover_r_out_front = FloatField('前部盖板外半径 (mm)', default=560.0)
    cover_thickness_front = FloatField('前部盖板厚度 (mm)', default=68.0)
    cover_r_out_behind = FloatField('后部盖板外半径 (mm)', default=940.0)
    cover_thickness_behind = FloatField('后部盖板厚度 (mm)', default=43.0)

    # 新增：法兰相关字段
    flange_r_in_front = FloatField('前部法兰内半径 (mm)', default=460.0)
    flange_r_out_front = FloatField('前部法兰外半径 (mm)', default=843.5)
    flange_thickness_front = FloatField('前部法兰厚度 (mm)', default=145.0)
    flange_slope_deg_front = FloatField('前部法兰倾斜角度 (度)', default=-84.88)
    flange_fillet_radius_front = FloatField('前部法兰圆角半径 (mm)', default=10.0)

    flange_r_in_behind = FloatField('后部法兰内半径 (mm)', default=815.0)
    flange_r_out_behind = FloatField('后部法兰外半径 (mm)', default=1258.72)
    flange_thickness_behind = FloatField('后部法兰厚度 (mm)', default=179.0)
    flange_slope_deg_behind = FloatField('后部法兰倾斜角度 (度)', default=92.78)
    flange_fillet_radius_behind = FloatField('后部法兰圆角半径 (mm)', default=10.0)

    # 新增：裙座相关字段
    skirt_r_out_front = FloatField('前部裙座外半径 (mm)', default=1835.5)
    skirt_r_in_1_front = FloatField('前部裙座内半径1 (mm)', default=1702.5)
    skirt_r_in_2_front = FloatField('前部裙座内半径2 (mm)', default=1797.585372)
    skirt_l_1_front = FloatField('前部裙座长度1 (mm)', default=23.0)
    skirt_l_2_front = FloatField('前部裙座长度2 (mm)', default=1650.0)
    skirt_offset_front = FloatField('前部裙座偏移量 (mm)', default=-450.0)

    skirt_r_out_behind = FloatField('后部裙座外半径 (mm)', default=1835.5)
    skirt_r_in_1_behind = FloatField('后部裙座内半径1 (mm)', default=1702.5)
    skirt_r_in_2_behind = FloatField('后部裙座内半径2 (mm)', default=1797.585372)
    skirt_l_1_behind = FloatField('后部裙座长度1 (mm)', default=23.0)
    skirt_l_2_behind = FloatField('后部裙座长度2 (mm)', default=1650.0)
    skirt_offset_behind = FloatField('后部裙座偏移量 (mm)', default=450.0)
