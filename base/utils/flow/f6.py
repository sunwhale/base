...
def create_part_block_a(model, part_name, points, lines, faces, dimension):
    ...
    # geom_list.append(s_block_partition.Line(point1=points[0, 2], point2=points[-1, 2]))
    # geom_list.append(s_block_partition.ArcByCenterEnds(center=center, point1=points[2, 0], point2=points[2, -1], direction=COUNTERCLOCKWISE))
...
def create_part_block_b(model, part_name, points, lines, faces, dimension):
    ...
    faces = p.faces.getByBoundingBox(0, tol, 0, x1 * 1.1, y1, length / 2.0)
    face_areas = []
    for face in faces:
        face_area = face.getSize()
        face_areas.append(face_area)
